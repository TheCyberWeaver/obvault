...
**Proof:** Basis
Consider $U\subseteq \mathbb{R}^{n},x \in U$. There exists some $r\in \mathbb{R}_{>0}$ such that $B_{r}(x)\subseteq U$. There is $q\in \mathbb{Q}_{>0}$ with $q\leq r$. By [[Math/Topology/Convergence#Definition dense|denseness]] there $p \in \mathbb{Q}^{n}$ such that $\lVert x-p \rVert< \frac{q}{2}$
...
- $A$ closed $\Longleftrightarrow$ $A$ contains every [[Math/Topology/Convergence#Definition Convergence|limit]] of every convergent sequence in $A$
